...
def condutas_por_diagnostico():
    ...
    for diagnostico, grupo in df.groupby(COL_CLASSE):
        ...
        conduta = {
            ...
            for coluna in colunas_conduta
            if coluna in linha and str(linha[coluna]).strip()
        }
        if "tipo_biopsia_procedimento" in conduta and "tipo_biopsia" not in conduta:
            conduta["tipo_biopsia"] = conduta["tipo_biopsia_procedimento"]
